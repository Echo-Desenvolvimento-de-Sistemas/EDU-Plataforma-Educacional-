...
import { Save, Filter, Loader2, Plus, ArrowLeft } from 'lucide-react';
...
const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Secretaria', href: '/secretaria/dashboard' },
    { title: 'Notas e Boletins', href: '/secretaria/grades' },
...
export default function Entry({ classes, subjects, gradingPeriods }: Props) {
    ...
    useEffect(() => {
        ...
        const classParam = urlParams.get('class_room_id');
        if (classParam && classes.some(c => c.id.toString() === classParam)) {
    ...
    const fetchData = async () => {
        setLoadingData(true);
        try {
            const response = await axios.get(`/secretaria/grades/${selectedClassId}/grades-api?grading_period_id=${selectedPeriodId}&subject_id=${selectedSubjectId}`);
    ...
    const handleSave = async () => {
        ...
        try {
            await axios.post(`/secretaria/grades/${selectedClassId}/grades-batch`, {
    ...
    const handleDeleteAssessment = async (assessmentId: number) => {
        ...
        try {
            await axios.delete(`/secretaria/grades/${selectedClassId}/assessments/${assessmentId}`);
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Lançar Notas - Secretaria" />
            ...
            <div className="flex flex-col gap-6 p-4 max-w-[95%] mx-auto w-full pb-20">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold tracking-tight">Lançamento de Notas (Secretaria)</h1>
                            <p className="text-muted-foreground">Insira ou edite as notas dos alunos em qualquer disciplina.</p>
                        </div>
                    </div>
                ...
                <DialogContent>
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={async () => {
                            ...
                            setIsCreating(true);
                            try {
                                const response = await axios.post(`/secretaria/grades/${selectedClassId}/assessments`, {
                                    ...createForm,
