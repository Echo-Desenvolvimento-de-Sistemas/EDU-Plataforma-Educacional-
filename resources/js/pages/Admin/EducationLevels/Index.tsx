...
export default function Index({ educationLevels }: Props) {
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Níveis de Ensino" />
            <div className="flex h-full flex-1 flex-col gap-4 overflow-hidden rounded-xl p-4">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Níveis de Ensino</h1>
                    <Link
                        href={admin.educationLevels.create.url()}
                        ...
                        <Card key={level.id}>
                            ...
                            <CardContent>
                                ...
                                <div className="flex justify-end gap-2">
                                    <Link
                                        href={admin.educationLevels.edit.url(level.id)}
                                        className="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-9 w-9"
                                    >
                                        <Edit className="h-4 w-4" />
                                    </Link>
                                    <Link
                                        href={admin.educationLevels.destroy.url(level.id)}
                                        method="delete"
                                        as="button"
                ...
                <div className="hidden sm:block flex-1 overflow-auto rounded-md border border-gray-200 dark:border-gray-700">
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-900">
                            {educationLevels.data.map((level) => (
                                <tr key={level.id}>
                                    <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900 dark:text-gray-100">{level.name}</td>
