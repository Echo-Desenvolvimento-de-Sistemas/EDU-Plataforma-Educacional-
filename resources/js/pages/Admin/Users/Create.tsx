...
interface ClassRoom {
    ...
    grade?: { id: number; education_level_id: number };
}
...
export default function Create({ classRooms, subjects, academicYears, educationLevels, grades }: Props) {
    ...
    const filteredClasses = useMemo(() => {
        return classRooms.filter(c => {
            ...
            // Also filter by level if grade not selected yet but level is?
            // Actually class -> grade -> level. So if level is selected, we should show classes whose grade belongs to that level
            // But usually we select grade first. Let's stick to Grade filter.
                ...
                <div className="mx-auto w-full max-w-4xl rounded-xl border border-sidebar-border/70 bg-white p-6 shadow-sm dark:border-sidebar-border dark:bg-sidebar-accent/10">
                    ...
                    <form onSubmit={submit}>
                                        ...
                                        <div>
                                            ...
                                            <div className="flex flex-wrap gap-1 mt-2">
                                                {selectedSubjects.map(id => {
                                                    ...
                                                    return sub ? <Badge key={id} variant="secondary" onClick={() => setSelectedSubjects(selectedSubjects.filter(i => i !== id))} className="cursor-pointer hover:bg-red-100 hover:text-red-600 dark:bg-gray-700 dark:text-gray-200">{sub.name} ×</Badge> : null;
                                                })}
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                            ...
                                            <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-900 dark:divide-gray-700">
                                                ...
                                                {data.allocations.length === 0 && (
                                                    <tr>
                                                        <td colSpan={5} className="px-4 py-8 text-center text-gray-500">Nhuma turma vinculada.</td>
                                                    </tr>
                                                )}
                                            </tbody>
                                        </table>
                        ...
                        <div className="flex items-center justify-end mt-6">
                            <Button className="ml-4 bg-blue-600 text-white hover:bg-blue-700" disabled={processing}>
                                Criar Usuário
                            </Button>
                        </div>
                    </form>
